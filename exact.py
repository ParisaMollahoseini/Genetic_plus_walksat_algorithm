...
def main():
    ...
    #b = solver.NumVar(-infinity, infinity, 'b')
    a = []
...
#define visited and set 1 to visited members in clauses 
    ...
    for i in range(0,max1+1):
        if visited[i] == 1:
            a[i] = solver.IntVar(0, 1, str('x{}'.format(i)))
#define a members 
    k=[]
    k=[0 for i in range(0,max1+1)]
    for i in range(0,max1+1):
        if visited[i] == 1:
            k[i] = solver.IntVar(1,infinity , str('k{}'.format(i)))
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        for i in range(0,max1+1):
            if visited[i] == 1:
                print(str('x{} ='.format(i)), a[i].solution_value())
